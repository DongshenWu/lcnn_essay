    ...
    def __init__(self,
        ...
        for _ in range(length):
            layers.append(get_conv(in_channels,
                                   in_channels,
                                   kernel_size))
            layers.append(get_activation(num_channels=in_channels))
...
class SimplifiedConvNet(nn.Sequential):
    """LipConvnet-style backbone: nrof_blocks of (conv, activation) ending in PixelUnshuffle(2)."""
...
def get_layers(hp: SimplifiedConvNetHyperparameters):
    ...
    kernel_sizes[-1] = 1  # final 2x2 block can't fit kernel_size>=3.
    downsize_blocks = [
        (f"Block{i + 1}", ConvBlock(
            hp.get_conv, hp.get_activation,
            hp.base_width * 2 ** i,
    ...
    layers = [
        ...
        ("FirstActivation", hp.get_activation(num_channels=hp.base_width)),
        *downsize_blocks,
...
DEFAULT_MODELS = OrderedDict([
    # 32x32 inputs (CIFAR / FashionMNIST)
    ...
    # 64x64 inputs (TinyImageNet); one extra block for the larger spatial dim.
    ('TConvNetXS', dict(nrof_layers_per_block=5, base_width=8,  nrof_blocks=6, kernel_size=3)),
    ('TConvNetS',  dict(nrof_layers_per_block=5, base_width=16, nrof_blocks=6, kernel_size=3)),
    ('TConvNetM',  dict(nrof_layers_per_block=5, base_width=32, nrof_blocks=6, kernel_size=3)),
    ('TConvNetL',  dict(nrof_layers_per_block=5, base_width=64, nrof_blocks=6, kernel_size=3)),
])
