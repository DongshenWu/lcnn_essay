...
class DeterministicLoader(SafeLoader):
    ...
    @staticmethod
    def activation_constructor(loader, node):
        """`!activation` returns a class (scalar form) or a partial bound with kwargs (mapping form);
        model code injects `num_channels` at the call site, since it varies per layer.
        """
        if isinstance(node, yaml.ScalarNode):
            classname = loader.construct_scalar(node)
            return getattr(models.layers, classname)
        kwargs = loader.construct_mapping(node, deep=True)
        classname = kwargs.pop('name')
        cls = getattr(models.layers, classname)
...
DeterministicLoader.add_constructor(
    "!activation", DeterministicLoader.activation_constructor)
