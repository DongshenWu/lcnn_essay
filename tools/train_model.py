...
def train_model(model, train_dataset):
    ...
    def get_optimizer(params):
        return torch.optim.SGD(params, lr=0., weight_decay=1e-5, momentum=0.9)

    def get_lr_scheduler(optimizer):
        return torch.optim.lr_scheduler.OneCycleLR(
            optimizer, max_lr=PEAK_LR, epochs=NROF_EPOCHS, steps_per_epoch=1)
    ...
    trainer = Train(
        model, train_dataset,
        batch_size=256, epochs=NROF_EPOCHS,
        loss=loss,
        device="cuda" if torch.cuda.is_available() else "cpu",
        optimizer=get_optimizer, lr_scheduler=get_lr_scheduler,
        valset=None, eval_metrics=eval_metrics,
        num_workers=4, logger=logging,
    )
    trainer.run()
