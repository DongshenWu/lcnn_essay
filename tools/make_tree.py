...
def get_header(dataset, mode):
    return f'''
## Auto-generated settings for {dataset['name']} ({mode}). Edit defaults.yml /
## epoch_budget.yml instead of this file.
'''
...
def get_epochs(epoch_budget: dict, model_id: str, conv_layer: str, mode: str, training_time: int):
    # epoch_budget is calibrated to 2-hour wall-clock; rescale linearly for final_training.
    if mode == 'random_search':
...
def create_directory_tree(root_dir: str, mode: str,
    ...
    if default_dataset in ('CIFAR10', 'CIFAR100', 'FashionMNIST'):
...
def arg_parser():
    ...
    parser.add_argument('--default', type=str, default='settings/defaults.yml',
                        help='Defaults YAML (dataset, optimizer, scheduler, loss).')
    parser.add_argument('--best-lr-wd-file', type=str, default=None,
                        help='CSV from eval_best_lr_wd.py; required for final_training.')
...
if __name__ == '__main__':
    create_directory_tree(**vars(arg_parser()))
